...
function log_verbose(...m) {
    if (!!process.env['VERBOSE_LOGS'])
        console.error('[pre_process_package_json.js]', ...m);
}
...
function clearYarnFilePathCaches(pkg) {
    ...
    if (clearPackages.length) {
        ...
        for (const c of clearPackages) {
            child_process.execFileSync('yarn', ['--mutex', 'network', 'cache', 'clean', c], { stdio: [process.stdin, process.stdout, process.stderr] });
        }
    }
}
module.exports = { main };
