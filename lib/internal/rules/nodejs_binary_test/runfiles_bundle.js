...
class Runfiles {
    ...
    constructor(_env) {
        ...
        if (!!_env['RUNFILES_MANIFEST_FILE']) {
            ...
            this.runfilesDir = path__namespace.resolve(_env['RUNFILES_DIR']);
        }
        else {
    ...
    loadRunfilesManifest(manifestPath) {
        ...
        const input = fs__namespace.readFileSync(manifestPath, { encoding: 'utf-8' });
    ...
    resolveWorkspaceRelative(modulePath) {
        ...
        return this.resolve(path__namespace.posix.join(this.workspace, modulePath));
    }
    ...
    resolvePackageRelative(modulePath) {
        ...
        return this.resolve(path__namespace.posix.join(this.workspace, this.package, modulePath));
    }
    /**
    ...
    _resolve(moduleBase, moduleTail) {
        if (this.manifest) {
            ...
            if (result) {
                if (moduleTail) {
                    const maybe = path__namespace.join(result, moduleTail || '');
                    if (fs__namespace.existsSync(maybe)) {
                        return maybe;
                    }
                }
                else {
        ...
        if (this.runfilesDir) {
            const maybe = path__namespace.join(this.runfilesDir, moduleBase, moduleTail || '');
            if (fs__namespace.existsSync(maybe)) {
                return maybe;
            }
        }
        const dirname = path__namespace.dirname(moduleBase);
        if (dirname == '.') {
            // no match
            return undefined;
        }
        return this._resolve(dirname, path__namespace.join(path__namespace.basename(moduleBase), moduleTail || ''));
    }
}
...
exports._BAZEL_OUT_REGEX = BAZEL_OUT_REGEX;
exports.runfiles = runfiles;
