...
export function patcher(fs: any = _fs, roots: string[]) {
  ...
  fs.readdir = (...args: any[]) => {
    ...
    cb = once(cb);
    args[args.length - 1] = (err: Error, result: any[]) => {
      if (err) {
        return cb(err);
      }
  ...
  function handleDirent(p: string, v: Dirent): Promise<Dirent> {
    ...
    return new Promise((resolve, reject) => {
      if (fs.DEBUG) {
        console.error(
          handleCounter + ' opendir: found link? ',
          path.join(p, v.name),
          v.isSymbolicLink(),
        );
      }
      ...
      origReadlink(linkName, (err: Error, target: string) => {
        ...
        if (fs.DEBUG) {
          console.error(
            handleCounter + ' opendir: escapes? [target]',
            path.resolve(target),
            '[link] ' + linkName,
            isEscape(path.resolve(target), linkName),
            roots,
          );
        }
...
function isOutPath(root: string, str: string) {
  return !root || (!str.startsWith(root + path.sep) && str !== root);
}

function escapeFunction(roots: string[]) {
