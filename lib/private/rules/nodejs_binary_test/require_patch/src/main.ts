...
type ModuleMapping = { module_name: RegExp, module_root: string };
type TemplateModuleMapping = { module_name: string, module_root: string };
 ...
 */
const MODULE_ROOTS: ModuleMapping[] = ([/*TEMPLATED_module_roots*/] as TemplateModuleMapping[])
  .sort((a, b) => b.module_name.length - a.module_name.length)
  .map<ModuleMapping>(a => ({ module_name: new RegExp(a.module_name), module_root: a.module_root }));
...
function resolveToModuleRoot(path: string) {
  if (!path) {
 ...
 */
function loadRunfilesManifest(manifestPath: string) {
...
function isDirectory(res: string) {
  try {
...
function loadAsFileSync(res: string) {
  if (isFile(res)) {
    return res;
  }
  if (isFile(res + '.mjs')) {
...
function loadAsDirectorySync(res: string): string|null {
...
function resolveManifestFile(res: string) {
  const maybe =
...
function resolveManifestDirectory(res: string) {
...
function resolveRunfiles(parent: string|undefined, ...pathSegments: string[]) {
...
module.constructor._resolveFilename =
    function(request: string, parent: { filename?: string }|null, isMain: boolean, options: any) {
