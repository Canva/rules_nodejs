...
process.env.PATH = [path_1.default.dirname(process.execPath), process.env.PATH].join(isWindows ? ';' : ':');
...
function log_verbose(...m) {
    ...
    if (VERBOSE_LOGS)
        console.error(`[${path_1.default.basename(__filename)}]`, ...m);
}
/**
 ...
 */
const MODULE_ROOTS = [ /*TEMPLATED_module_roots*/]
    .sort((a, b) => b.module_name.length - a.module_name.length)
    .map(a => ({ module_name: new RegExp(a.module_name), module_root: a.module_root }));
...
function loadRunfilesManifest(manifestPath) {
    ...
    const input = fs_1.default.readFileSync(manifestPath, { encoding: 'utf-8' });
...
function isFile(res) {
    try {
        return fs_1.default.statSync(res).isFile();
    }
    catch (e) {
        return false;
    }
...
function isDirectory(res) {
    try {
        return fs_1.default.statSync(res).isDirectory();
    }
    catch (e) {
        return false;
    }
...
function readDir(dir) {
    return fs_1.default.statSync(dir).isDirectory()
        ? Array.prototype.concat(...fs_1.default.readdirSync(dir).map(f => readDir(path_1.default.join(dir, f))))
        : [dir.replace(/\\/g, '/')];
}
...
function loadAsDirectorySync(res) {
    const pkgfile = path_1.default.join(res, 'package.json');
    if (isFile(pkgfile)) {
        try {
            const pkg = JSON.parse(fs_1.default.readFileSync(pkgfile, 'utf-8'));
            let main = pkg['main'];
            if (main) {
                if (main === '.' || main === './') {
                    main = 'index';
                }
                let maybe = loadAsFileSync(path_1.default.resolve(res, main));
                if (maybe) {
                    return maybe;
                }
                maybe = loadAsDirectorySync(path_1.default.resolve(res, main));
                if (maybe) {
                    return maybe;
                }
            }
    ...
    return loadAsFileSync(path_1.default.resolve(res, 'index'));
}
function resolveManifestFile(res) {
    const maybe = runfilesManifest[res] || runfilesManifest[res + '.mjs'] || runfilesManifest[res + '.js'];
    if (maybe) {
        return maybe;
    }
    ...
    while (segments.length) {
        ...
        if (tree && isDirectory(tree)) {
            ...
            const files = readDir(tree).map(f => path_1.default.relative(tree, f).replace(/\\/g, '/'));
            files.forEach(f => {
                runfilesManifest[path_1.default.posix.join(test, f)] = path_1.default.posix.join(tree, f);
            });
...
function resolveManifestDirectory(res) {
    const pkgfile = runfilesManifest[path_1.default.posix.join(res, 'package.json')];
    if (pkgfile) {
        try {
            const pkg = JSON.parse(fs_1.default.readFileSync(pkgfile, 'utf-8'));
            let main = pkg['main'];
            if (main) {
                ...
                let maybe = resolveManifestFile(path_1.default.posix.join(res, main));
                if (maybe) {
                    return maybe;
                }
                maybe = resolveManifestDirectory(path_1.default.posix.join(res, main));
                if (maybe) {
                    return maybe;
                }
            }
    ...
    return resolveManifestFile(path_1.default.posix.join(res, 'index'));
}
function resolveRunfiles(parent, ...pathSegments) {
    ...
    const runfilesPath = path_1.default.join(process.env.RUNFILES, runfilesEntry);
    if (runfilesManifest) {
        if (parent && runfilesEntry.startsWith('.')) {
            ...
            if (parentRunfile) {
                runfilesEntry = path_1.default.join(path_1.default.dirname(parentRunfile), runfilesEntry);
            }
        }
        ...
        runfilesEntry = path_1.default.normalize(runfilesEntry).replace(/\\/g, '/');
    ...
    function (request, parent, isMain, options) {
        ...
        if (!isMain) {
            ...
            if (moduleRoot) {
                ...
                if (filename) {
                    ...
                    failedResolutions.push(`module root ${moduleRoot} - No file ${request} found in module root ${moduleRoot}`);
                }
            }
        }
        ...
        try {
            ...
            if (resolved === request || request.startsWith('.') || request.startsWith('/') ||
                request.match(/^[A-Z]\:[\\\/]/i)) {
                if (SILLY_VERBOSE)
                    log_verbose(`resolved ${request} to built-in, relative or absolute import ` +
                ...
                if (parentNodeModulesSegment != -1) {
                    ...
                    const relative = path_1.default.relative(parentRoot, resolved);
                    if (!relative.startsWith('..')) {
                        // Resolved within parent node_modules
                        log_verbose(`resolved ${request} within parent node_modules to ` +
                            `${resolved} from ${parentFilename}`);
                        return resolved;
                    }
                    else {
                        throw new Error(`Resolved to ${resolved} outside of parent node_modules ${parentFilename}`);
                    }
                }
        ...
        let relativeParentFilename = parentFilename ? path_1.default.relative(process.env.RUNFILES, parent.filename) : undefined;
        if (relativeParentFilename && !relativeParentFilename.startsWith('..')) {
            ...
            if (parentSegments[0] !== USER_WORKSPACE_NAME) {
                try {
                    ...
                    log_verbose(`resolved ${request} within node_modules ` +
        ...
        try {
            ...
            log_verbose(`resolved ${request} within node_modules (${NODE_MODULES_ROOT}) to ` +
                `${resolved} from ${parentFilename}`);
            return resolved;
        }
        catch (e) {
...
// source-map-support.
try {
    const sourcemap_support_package = path_1.default.resolve(process.cwd(), '../build_bazel_rules_nodejs/third_party/github.com/source-map-support');
